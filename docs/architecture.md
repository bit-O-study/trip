...
- **양방향 하이라이트** — 타임라인 항목을 선택하면 해당 마커가 확대되고 지도가 그 지점으로 이동한다. 마커를 누르면 타임라인 항목으로 스크롤·강조되고 포커스가 옮겨간다. 같은 항목을 다시 누르거나 `Esc`로 해제한다.

  > **정정(2026-08-20).** 초안의 "hover 시 마커 확대"는 채택하지 않았다. 이 앱은 모바일 우선이고 터치에는 hover가 없다. hover를 기준으로 잡으면 주 사용 환경에서 이 기능이 통째로 사라지므로, 두 방향 모두 **클릭·포커스**를 기준으로 한다. 데스크톱 hover는 보조 강조로 나중에 얹을 수 있지만 동작의 근거는 아니다.
