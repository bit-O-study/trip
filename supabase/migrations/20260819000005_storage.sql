...
-- trip.attachments 의 CHECK 제약이 같은 규칙을 강제하므로 두 곳이 어긋나지 않는다.
...
create or replace function trip_private.trip_id_from_storage_path(p_name text)
returns uuid
language plpgsql
immutable
set search_path = ''
...
revoke all on function trip_private.trip_id_from_storage_path(text) from public, anon;
grant execute on function trip_private.trip_id_from_storage_path(text) to authenticated;
