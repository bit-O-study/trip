...
export function DayItemAdd({ tripId, date, timezone, polls, initialPollId, initialQuery }: Props) {
  const [mode, setMode] = useState<"place" | "manual" | null>(initialPollId ? "place" : null);
  if (!mode) {
  ...
  return <div id={initialPollId ? "poll-candidate-add" : undefined} className="scroll-mt-24 space-y-3 rounded-xl border border-border bg-card p-3">
    ...
    {mode === "place" ? <PlaceSearch tripId={tripId} defaultDate={date} timezone={timezone} polls={polls} initialPollId={initialPollId} initialQuery={initialQuery} /> : <ItemForm tripId={tripId} defaultDate={date} timezone={timezone} defaultOpen onCancel={() => setMode(null)} />}
  </div>;
}
