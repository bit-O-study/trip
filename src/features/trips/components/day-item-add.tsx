...
export function DayItemAdd({ tripId, date, timezone, polls, initialPollId }: Props) {
  const [mode, setMode] = useState<"place" | "manual" | null>(null);
  if (!mode) {
  ...
  return <div className="space-y-3 rounded-xl border border-border bg-card p-3">
    ...
    {mode === "place" ? <PlaceSearch tripId={tripId} defaultDate={date} timezone={timezone} polls={polls} initialPollId={initialPollId} /> : <ItemForm tripId={tripId} defaultDate={date} timezone={timezone} defaultOpen onCancel={() => setMode(null)} />}
  </div>;
}
